...
AGV_PLATE_FULL_POSE = Pose(
    position=Point(-0.128, -0.359, 0.14), orientation=GRIPPER_ORIENTATION
)  # AGV满料盘位置
AGV_PLATE_EMPTY_POSE = Pose(
    position=Point(0.117, -0.361, 0.14), orientation=GRIPPER_ORIENTATION
)  # AGV空料盘位置
SAFETY_HEIGHT = 0.32
...
SMF_TABLE_BOX_OFFSET_X = 0.267
SMF_TABLE_PEN_OFFSET_X = 0.267 - 0.27
SMF_TABLE_OFFSET_Y = 0.152
CAMERA_GRIPPER_OFFSET = 0.066
PLATE_HEIGHT = 0.025

# 速度常量
PTP_SCALE = 0.2  # 点到点移动速度比例
LIN_SCALE = 0.1  # 直线移动速度
PNP_SCALE = 0.02  # 拾取与放置速度

# 发送至PSS Modbus寄存器地址
pss_modbus_write_dic = {"agv_ros_program_run": 1000}
...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
        ...
        if 0 <= box_plate_place_number < 5:
            ...
            place_height = (
                FEED_TABLE_PNP_OFFSET_Z + (5 - box_plate_place_number) * 0.025 - 0.023
            )
            r.move(
                Lin(
        ...
        if 0 <= pen_plate_place_number < 5:
            ...
            place_start_pose = r.get_current_pose()
            place_height = (
                FEED_TABLE_PNP_OFFSET_Z + (5 - pen_plate_place_number) * 0.025 - 0.023
            )
            r.move(
                Lin(
                    goal=Pose(position=Point(0, 0, place_height)),
